...
**A. From Salesforce (recommended — no Azure config).** Store the credentials on the eSign **External Credential** and let the Named Credential inject them as headers. Apex never reads the secret; it travels the same path as the existing `X-API-Key`.
...
2. On the External Credential `eSign_API`, add custom auth parameters on the principal: `MSTenantId`, `MSClientId`, `MSClientSecret`, `MSUserId`.
3. On the Named Credential `eSign_API`, add custom headers:
   - `X-MS-Tenant-Id: {!$Credential.eSign_API.MSTenantId}`
   - `X-MS-Client-Id: {!$Credential.eSign_API.MSClientId}`
   - `X-MS-Client-Secret: {!$Credential.eSign_API.MSClientSecret}`
   - `X-MS-User-Id: {!$Credential.eSign_API.MSUserId}`

**B. From Azure env vars.** Set `MS_TENANT_ID`, `MS_CLIENT_ID`, `MS_CLIENT_SECRET` (secret), `MS_USER_ID` on the Container App. Use this when the customer requires the Graph secret never leave their Azure boundary.

Verify: a generation log line `Graph API conversion: … bytes PDF` means Graph is active; `falling back to LibreOffice` means the config was incomplete (signing still works).
...
- Optional Microsoft Graph credentials are read from request headers (injected by Salesforce) or env vars; the secret is never read by Apex and never logged.
